...
func Nai4WithFormat(w http.ResponseWriter, r *http.Request, req config.ChatRequest, randomSeed int, base64String string, authHeader string, cfg *config.Config, userInput string, characterPrompts []CharacterPrompt, isDallRequest bool, responseFormat string) {
	Nai4WithFormatAndSize(w, r, req, randomSeed, base64String, authHeader, cfg, userInput, characterPrompts, cfg.Parameters.Width, cfg.Parameters.Height, isDallRequest, responseFormat)
}

func Nai4WithFormatAndSize(w http.ResponseWriter, r *http.Request, req config.ChatRequest, randomSeed int, base64String string, authHeader string, cfg *config.Config, userInput string, characterPrompts []CharacterPrompt, width int, height int, isDallRequest bool, responseFormat ...string) {
	// 请求连接
